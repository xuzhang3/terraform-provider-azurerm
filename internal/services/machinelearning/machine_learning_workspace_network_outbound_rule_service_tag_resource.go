...
func (r WorkspaceNetworkOutboundRuleServiceTag) Update() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		Timeout: 30 * time.Minute,
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			var model managednetwork.ServiceTagDestination
			if err := metadata.Decode(&model); err != nil {
				return fmt.Errorf("decoding: %+v", err)
			}
			...
			if metadata.ResourceData.HasChange("service_tag") {
				serviceTagOutboundRule.Destination.ServiceTag = model.ServiceTag
			}

			if metadata.ResourceData.HasChange("protocol") {
				serviceTagOutboundRule.Destination.ServiceTag = model.Protocol
			}

			if metadata.ResourceData.HasChange("port_ranges") {
				serviceTagOutboundRule.Destination.PortRanges = model.PortRanges
			}
